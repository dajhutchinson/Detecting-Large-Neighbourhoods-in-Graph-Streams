...
int main() {
  ...
  // details of graph to perform on
  edge_file_path="../../data/facebook_deletion.edges"; vertex_file_path="../../data/facebook_deletion.vertices"; out_file="facebook_results.csv";
  num_vertices=747; d=267; reps=10;
  execute_test(2,20,1,reps,d,num_vertices,edge_file_path,vertex_file_path,out_file);
...
void execute_test(int c_min, int c_max, int c_step, int reps, int d, int n, string edge_file_path, string vertex_file_path, string out_file) {
  ...
  outfile<<"name,"<<vertex_file_path<<endl<<"n,"<<n<<endl<<"d,"<<d<<endl<<"repetitions,"<<reps<<endl<<endl; // test details
  outfile<<"c,time (microseconds), generatig l0 hash time (microseconds) ,mean max space (bytes), l0 hash space (bytes), variance time, variance hash time, variance max space, variance hash space,successes"<<endl; // headers
  set<vertex> neighbourhood; vertex root; // variables for returned values
  ...
  for (int c=c_min;c<=c_max;c+=c_step) {
    ...
    cout<<"\rCALCULATED MEAN             ";
    double variance_duration   =variance(times);
    cout<<"\rCALCULATED MEAN 1";
...
void single_pass_insertion_deletion_stream(int c, int d, int num_vertices, string edge_file_path, string vertex_file_path, set<vertex>& neighbourhood, vertex& root) {
  ...
  double delta=0.1, gamma=0.1; // success_rate ~= P(L0 sampler returning a vertex given delta & gamma)

  // calculate model feature
  int x=(sqrt(num_vertices)>num_vertices/c) ? sqrt(num_vertices) : num_vertices/c; // size of vertex sample
