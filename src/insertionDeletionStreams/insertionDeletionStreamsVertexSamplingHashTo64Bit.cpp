...
void generate_random_hash(int n, int64_t m, map<vertex,int64_t>& hash);
...
int main() {
  ...
  string edge_file_path="../../data/artifical/doublepoisson05ID.edges";
  string vertex_file_path="../../data/artifical/doublepoisson05ID.vertices";
  ...
  int reasonable_degree=51; // reasonable expectation for the degree of a node
  ...
  map<vertex,int64_t>* unique_hash_maps=new map<vertex,int64_t>[total_samplers];
  int64_t n3=pow(num_vertices,3);
  for (int i=0; i<total_samplers; i++) {
    if (i%10==0) cout<<"\r"<<i;
    map<vertex,int64_t> hash_map;
    generate_random_hash(num_vertices,n3,hash_map);
  ...
  while (getline(edge_stream,line)) {
    ...
    for (int i=0; i<total_samplers; i++) { // update every l0 sampler
      ...
      if (v!=-1) { // edge is connected to current target
        ...
        int64_t h=unique_hash_maps[i][v]; // update certain s-sparse recoveries
