...
int main() {
  int c=100, d=400; // c=runs, d/c=d2
  vector<int>* ptrs[c]; // pointers to each generated neighbourhood
  vector<int> success; // indices of successes

  for (int i=0; i<c; i++) { // step d1 each iteration
    ifstream stream("data/facebook.edges"); // load stream
    vector<int>* n=new vector<int>; // generate vector for neighbourhood
    ptrs[i]=n; // store pointer
    deg_res_sampling(max(1,(i*d)/c),d/c,10,stream,*n); // NB does not tell you whose neighbourhood it is
    if (n->size()==d/c) success.push_back(i); // record index if successful run
  }

  if (success.size()>0) { // if there were any successes
    int i=rand()%success.size(); // generate index
    vector<int>* n=ptrs[success[i]]; // pointer to neighbourhood
    cout<<"RUN - "<<i<<"("<<max(1,(i*d)/c)<<","<<d/c<<")"<<endl; // details of runs
    for (vector<int>::iterator i=n->begin(); i!=n->end(); i++) cout<<*i<<endl; // output neighbourhood
    return 0;
  } else { // no successes
